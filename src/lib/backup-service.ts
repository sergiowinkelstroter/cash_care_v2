...
async function restoreUserData(userId: number, userData: any) {
  await prisma.$transaction(async (tx) => {
    await tx.categoryToUnit.deleteMany({ where: { userId } });
    await tx.installment.deleteMany({ where: { userId } });
    await tx.movement.deleteMany({ where: { userId } });
    await tx.payable.deleteMany({ where: { userId } });
    ...
    await tx.unit.createMany({ data: userData.Unit });
    await tx.category.createMany({ data: userData.Category });
    await tx.movement.createMany({ data: userData.Movement });
    await tx.payable.createMany({ data: userData.Payable });
    await tx.installment.createMany({ data: userData.Installment });
    await tx.categoryToUnit.createMany({ data: userData.CategoryToUnit });
  });
}
